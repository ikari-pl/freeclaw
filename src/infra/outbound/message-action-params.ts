...
export function resolveTelegramAutoThreadId(params: {
  ...
  // Only auto-inject threadId for forum topics (`:topic:` in channelId).
  // DM channels have currentThreadTs as a reply-chain message_id, not a
  // forum topic — injecting it causes "message thread not found" errors.
  if (!context.currentChannelId.includes(":topic:")) {
    return undefined;
  }
